...
const
  ...
  CNum2: array[1..10] of string = ('vingt','trente','quarante','cinquante',
                         'soixante','soixante-dix','quatre-vingt','septante',
                         'octante', 'nonante');
...
function TValue2StBelg.Decade2St(const AValue: Integer; Plural: Boolean
  ): string;
begin
  Result:=inherited Decade2St(AValue, Plural);
  case AValue of // on ne modifie que les nouvelles valeurs
    70, 80, 90 : Result := CNum2[(AValue div 10) + 1] ;
    71, 81, 91 : Result := CNum2[(AValue div 10) + 1] + '-et-un' ;
